...
RSpec.describe Packages::Dependency, type: :model do
  ...
  describe '.for_names_and_version_patterns' do
    let!(:package_dependency1) { create(:packages_dependency, name: "foo", version_pattern: "~1.0.0") }
    let!(:package_dependency2) { create(:packages_dependency, package: package_dependency1.package, name: "bar", version_pattern: "~2.5.0") }
    let(:names) { %w[foo bar] }
    let(:version_patterns) { %w[~1.0.0 ~2.5.0] }

    subject { Packages::Dependency.for_names_and_version_patterns(names, version_patterns) }
    ...
    context 'with parameters size' do
      ...
      let!(:package_dependency6) { create(:packages_dependency, package: package_dependency1.package, name: "foo6", version_pattern: "~1.5.6") }
