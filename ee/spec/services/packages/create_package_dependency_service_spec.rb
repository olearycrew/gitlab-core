...
describe Packages::CreatePackageDependencyService do
  describe '#execute' do
    ...
    context 'when packages are published' do
      ...
      let(:dependency_names) { package.package_dependencies.map(&:name).sort }
      let(:dependency_link_types) { package.package_dependency_links.map(&:dependency_type).sort }

      subject { described_class.new(package, package_dependencies).execute }

      it 'creates package dependencies and links' do
        expect { subject }
          .to change { Packages::PackageDependency.count }.by(1)
          .and change { Packages::PackageDependencyLink.count }.by(1)
      ...
      context 'with repeated packages' do
        ...
        it 'creates package dependencies and links' do
          expect { subject }
            .to change { Packages::PackageDependency.count }.by(4)
