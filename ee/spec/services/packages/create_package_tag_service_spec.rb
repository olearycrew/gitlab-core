...
describe Packages::CreatePackageTagService do
  ...
  describe '#execute' do
    ...
    shared_examples 'it creates the tag' do
      ...
      it 'adds tag to the package' do
        tag = subject
        expect(package.reload.package_tags).to match_array([tag])
      end
    end
    ...
    context 'with exisiting tag name' do
      let!(:package_tag2) { create(:package_tag, package: package2, name: tag_name) }
      ...
      context 'on same package with different version' do
        ...
        it { expect { subject }.to not_change { Packages::PackageTag.count } }
        it { expect(subject.name).to eq(tag_name) }
        it 'adds tag to the package' do
          tag = subject
          expect(package.reload.package_tags).to match_array([tag])
