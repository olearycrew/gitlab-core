...
class CreatePackagesPackageDependencyLinks < ActiveRecord::Migration[5.2]
  ...
  def change
    ...
    add_index :packages_package_dependency_links, [:package_id, :package_dependency_id, :dependency_type], unique: true, name: 'idx_pkgs_package_dep_links_on_pkg_id+pkg_dependency_id+dep_type'
  end
end
